...
def merge(
    ...
    if method in MERGE_METHODS:
        ...
        raise ValueError(
            f"Unknown method {method}, must be one of {list(MERGE_METHODS.keys())} or callable"
        )
    ...
    for idx, dataset in enumerate(datasets):
        ...
        # region_mask = true where there is NODATA
        if math.isnan(nodataval):
        ...
        valid_mask = dest != nodataval

    if dst_path is None:
        return dest, output_transform, valid_mask
