...
class XView2StackSampleCollateFn:
    def __init__(self):
        pass

    def __call__(self, samples):
        imgs = stack_samples(samples)["image"]
        return imgs, None
...
class XView2(NonGeoDataset):
    ...
    * https://arxiv.org/abs/1911.09296
    ...
    def __getitem__(self, index: int) -> Dict[str, Tensor]:
        ...
        pre = self._load_image(files["pre"])
        post = self._load_image(files["post"])
        mask_pre = self._load_target(files["mask_pre"])
        mask_post = self._load_target(files["mask_post"])

        image = torch.stack(tensors=[pre, post], dim=0)
        mask = torch.stack(tensors=[mask_pre, mask_post], dim=0)
    ...
    def _load_files(self, root: str, split: str) -> List[Dict[str, str]]:
        ...
        for name in set(basenames):
            pre = os.path.join(image_root, f"{name}_pre_disaster.png")
            post = os.path.join(image_root, f"{name}_post_disaster.png")
            mask_pre = os.path.join(mask_root, f"{name}_pre_disaster_target.png")
            mask_post = os.path.join(mask_root, f"{name}_post_disaster_target.png")
            files.append(
                dict(pre=pre, post=post, mask_pre=mask_pre, mask_post=mask_post)
            )
        return files
    ...
    def plot(
        ...
        """
        ncols = 2
        image_pre = draw_semantic_segmentation_masks(
            sample["image"][0], sample["mask"][0], alpha=alpha, colors=self.colormap
        )
        image_post = draw_semantic_segmentation_masks(
...
def build_xview2_sampler(config, num_replicas, rank, transforms, split="train"):
    xv_dataset = XView2(
        root=config["data"]["img_dir"], split=split, transforms=transforms
    )
    ...
    collate_function = XView2StackSampleCollateFn()
    return xv_dataset, sampler, collate_function
